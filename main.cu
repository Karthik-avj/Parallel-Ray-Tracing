...
__device__ void sphere_intersection(float* origin, float* ray_direction,
...
__device__ void nearest_intersection_object(float *objects, float *origin, int *object_n, float *ray_direction, int *objects_len, float *min_dist, int *object_idx){
	float distances[4];
...
__device__ void shadowed(int *is_shad, float *normal, float *light_dir, float *min_dist, float *origin, float *ray_dir,
...
__device__ void color(float* normal_surface, float* light_intersection, float* ray_dir, float* object, float* light, float* illumination){
...
__global__ void single_pixel(float* objects, int* objects_len, float* lights, float* camera, int* image){
...
int main(){
    ...
    single_pixel<<<gridDim, blockDim>>>(dev_objects, dev_objects_len, dev_light, dev_camera, dev_image);
    ...
    for(int j=N-1; j>=0;j--){
        for(int i=0; i<N; i++){
            printf("%d %d %d\n", image[3*N*i+3*j+0], image[3*N*i+3*j+1], image[3*N*i+3*j+2]);
        }
    }
}